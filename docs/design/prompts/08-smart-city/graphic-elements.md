...
**Payload:** consent manufactured by default. Sponsor = SMART CITY / BY USING THIS CITY YOU AGREE. Number from a tracking ID. Nameplate = USER / DATA SUBJECT.

**Palette:** rain `#1c2b33` · cyan glow `#21d9c9` · bone `#EDE6D8` · ink `#0E0E0E`.

**MJ settings:** `--ar 16:9 --style raw` (no `--v`).

> ⚠️ **Receipt note (Prompt 8):** hem carries a satirical EULA clause (invented-copy device, not a factual claim). Swap in a sourced + cited surveillance fact if preferred.
...
surveillance / civic-tech UI aesthetic,
...
cyan and bone outline on dark,
official badge / surveillance aesthetic,
white background --ar 16:9 --style raw
```
...
clean tech sans-serif, cyan on wet-asphalt dark rectangle,
civic-tech startup branding aesthetic,
white background --ar 16:9 --style raw
```
...
small caps under the main mark, bone on dark,
terms-of-service fine-print aesthetic,
white background --ar 16:9 --style raw
```
...
cyan border, dark field, camera-lens center,
...
tiny monospace fine-print, bone on dark,
terms-of-service microprint, hem / care-label placement,
white background --ar 16:9 --style raw
```

### Prompt 9: Maker mark + trophy-misuse
```
two small graphics:
a maker band reading "TERMS APPLY",
and a World Cup trophy silhouette restyled as a CCTV camera on a pole / a server rack,
cyan line art on dark,
surveillance aesthetic, collar / sleeve accent,
white background --ar 16:9 --style raw
```
