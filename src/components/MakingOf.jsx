...
function Shell({ children }) {
  return <div className="grain min-h-screen bg-bone text-ink">{children}</div>
}
...
function Detail({ slug }) {
  ...
  if (!data) return <Shell><div className="flex min-h-screen items-center justify-center text-[11px] uppercase tracking-[0.3em] text-ink/40">Loading…</div></Shell>

  return (
    <Shell>
      <section className="mx-auto max-w-6xl px-6 pt-14 pb-6 md:pt-20">
      ...
      {data.stages.map((st, i) => (
        <section key={st.key} className={'border-t border-ink/10 ' + (i % 2 ? 'bg-rain/10' : '')}>
