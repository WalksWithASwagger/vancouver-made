...
import { stages, thesis, HANDS } from '../data/process.js'
...
// The pipeline at a glance — human → handoff → machine → human gate.
function Rail() {
  return (
...
function Station({ s }) {
  return (
    <article id={`stage-${s.key}`} className="scroll-mt-6 border-t border-bone/10 py-10 md:py-14">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {s.images.map((img) => (
                <figure key={img.src} className="border border-bone/15 bg-rain/20">
                  ...
                  <figcaption className="border-t border-bone/10 px-2 py-1.5 font-mono text-[10px] uppercase tracking-wider text-bone/50">
                    {img.caption}
                  </figcaption>
                </figure>
              ))}
  ...
  return (
    <div className="grain min-h-screen bg-ink text-bone">
      ...
      <footer className="border-t border-bone/10 px-6 py-10 text-center text-xs uppercase tracking-[0.3em] text-bone/40">
        ...
        Voice in, judgment out — the machine in between. The receipts hold the line.
      </footer>
    </div>
  )
}
