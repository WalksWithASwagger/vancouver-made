  ...
  return (
    <div className="grain min-h-screen bg-ink text-bone">
      <header className="border-b border-bone/10 px-6 py-10 md:px-10">
        ...
        <h1 className="headline text-4xl text-bone md:text-6xl">THE <span className="text-hazard">GALLERY</span></h1>
        ...
        <div className="mt-5 flex flex-wrap gap-2 text-xs uppercase tracking-[0.2em]">
          <button
            ...
            className={'rounded border px-3 py-1 transition ' + (filter === 'all' ? 'border-hazard bg-hazard/10 text-hazard' : 'border-bone/20 text-bone/60 hover:text-bone')}
          >
            All · {total}
          </button>
          {groups.map((g) => (
            <button
              ...
              className={'rounded border px-3 py-1 transition ' + (filter === g.concept ? 'border-hazard bg-hazard/10 text-hazard' : 'border-bone/20 text-bone/60 hover:text-bone')}
            >
              {g.concept}
            </button>
          ))}
      ...
      <main className="px-6 py-10 md:px-10">
          ...
          return (
            <section key={g.concept} className="mb-16">
              <div className="mb-5 max-w-2xl">
                ...
                <p className="mt-1 text-sm text-bone/70">{g.blurb}</p>
              </div>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                {g.items.map((it, i) => (
                  <figure
                    ...
                    className="group cursor-pointer overflow-hidden rounded-lg border border-bone/12 bg-black/40"
                  >
                    <img
                      src={it.src}
                      alt={it.caption}
                    ...
                    />
                    <figcaption className="px-2 py-2 text-[11px] leading-snug text-bone/55">{it.caption}</figcaption>
                  </figure>
                ))}
              </div>
            </section>
        ...
        <div className="border-t border-bone/10 pt-10 text-center">
          <Link to="/journey" className="text-xs uppercase tracking-[0.2em] text-bone/60 transition hover:text-hazard">
            ← back to the journey
          </Link>
        </div>
      </main>
      ...
      {cur && (
        ...
        >
          <button
            ...
            className="absolute left-3 top-1/2 -translate-y-1/2 px-3 py-2 text-2xl text-bone/50 hover:text-bone"
            aria-label="Previous"
          >‹</button>
          <figure onClick={(e) => e.stopPropagation()} className="flex max-h-full max-w-5xl flex-col items-center">
            ...
            <figcaption className="mt-3 text-center text-xs uppercase tracking-[0.15em] text-bone/60">
              {cur.concept} · {cur.caption}
            </figcaption>
          </figure>
          <button
            onClick={(e) => { e.stopPropagation(); setActive((i) => (i + 1) % flat.length) }}
            className="absolute right-3 top-1/2 -translate-y-1/2 px-3 py-2 text-2xl text-bone/50 hover:text-bone"
            aria-label="Next"
          >›</button>
          <button
            onClick={() => setActive(null)}
            className="absolute right-4 top-4 text-xs uppercase tracking-[0.2em] text-bone/50 hover:text-bone"
            aria-label="Close"
          >Close ✕</button>
        </div>
      )}
